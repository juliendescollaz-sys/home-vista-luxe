...
export function GroupWizard({ open, onOpenChange }: GroupWizardProps) {
  ...
  const handleCreate = async () => {
    if (selectedDomains.length === 0) return;
    try {
      await createOrUpdateGroup({ name, domain: selectedDomains[0] as HaGroupDomain, domains: selectedDomains, entityIds: selectedEntityIds, scope: isShared ? "shared" : "local" });
