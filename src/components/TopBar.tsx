...
export const TopBar = ({
  ...
  return <>
      <div className="fixed top-0 left-0 right-0 glass-nav border-b border-border/30 z-40" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
        <div className="max-w-screen-xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className={`${isLeftHanded && isMobile ? "order-2" : "order-1"}`}>
            <img src={theme === "light" ? neoliaLogoDark : neoliaLogoLight} alt="Neolia" className="h-8 w-auto" />
          </div>

          {/* Titre centré - uniquement pour tablet/panel */}
          {title && !isMobile && <h1 className="absolute left-1/2 -translate-x-1/2 text-2xl font-bold">
              {title}
            </h1>}
      ...
      {isMobile && <MobileMenu open={menuOpen} onOpenChange={setMenuOpen} />}
    </>;
};